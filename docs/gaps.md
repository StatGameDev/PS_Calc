...
| G56 | [ ] | DPS: skill timing calculator missing. Data is already in skills.json (cast_time, after_cast_act_delay per level scraped from skill_db.conf). Need a `calculate_skill_timing()` function and wire into `calculate_dps()`. All mechanics source-confirmed — see `docs/skill_timing.md`. Session: Q0-Timing. | future |
| G59 | [ ] | Cast/delay gear bonuses not in pipeline: `sd->castrate` (bonus bCastrate; in pre-re bVarCastrate is identical — pc.c:2639) applies in skill_castfix ~line 17197; `sd->delayrate` (bonus bDelayrate — pc.c:3020) applies in skill_delay_fix ~line 17506. `bonus2 bCastrate,skill_id,val` per-skill cast rate stored in `sd->skillfixcastrate[]` (pc.c:3607). None of these are parsed by item_script_parser.py or present in GearBonuses. See `docs/skill_timing.md` SD->castrate / SD->delayrate sections. | Q0 |
| G60 | [ ] | SC_SUFFRAGIUM (Priest PR_SUFFRAGIUM) not implemented as a buff. Reduces cast time by val2 = 15×lv % (status.c:8485); consumed on cast (status_change_end in skill_castfix_sc skill.c:17244). Needs: buff toggle in support_buffs + Party Buffs UI row, and wiring in calculate_skill_timing(). See `docs/skill_timing.md`. | Q0 |
