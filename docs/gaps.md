...
| G52 | [~] | Dual-wield pipeline: AS_RIGHT/AS_LEFT passive rows, LH forge fields on PlayerBuild, per-slot forge widgets in equipment_section, lh_normal/lh_crit on BattleResult, dual-wield branch in BattlePipeline (RH×(50+AS_RIGHT×10)/100, LH×(30+AS_LEFT×10)/100, floor 1, battle.c:5923-5938), LH card browser EQP fix, monster perfect_dodge=0. REMAINING: TF_DOUBLE/GS_CHAINACTION proc behaviour during dual-wield — needs Hercules source read (does proc fire RH-only or per-hand? how does proc_hit_count interact with LH branch?). | G52 |
...
| G56 | [ ] | DPS: skill timing data missing. cast + delay fields from skill_db.conf not scraped into skills.json. Without them, DPS for skill attacks uses ASPD-based auto-attack delay (misleading for any non-auto-attack selection). Add fields to import_skill_db.py. Required before DPS is meaningful for Q1/Q2 skill selections. | future |
