...
| G77 | [ ] | PR_LEXAETERNA ×2 only applies to magic (magic_pipeline.py). Physical BF_WEAPON hits with Lex Aeterna active are not doubled. In Hercules, SC_LEXAETERNA doubles the next hit regardless of type (battle.c). Needs detection in BattlePipeline._run_branch() and ×2 multiplier applied after FinalRateBonus. |
