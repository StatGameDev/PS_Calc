...
| G86 | [ ] | **Regression from SkillParam-Refactor**: combat→buffs sphere sync was dropped by mistake. Changing the sphere count in the combat skill-params widget should push back to the MO_SPIRITBALL dropdown in Self Buffs (bidirectional sync, as in the pre-refactor code). Fix: restore `spirit_spheres_changed = Signal(int)` on `CombatControlsSection` (or emit the equivalent), connect it in `main_window._connect_combat_signals`, and update `BuffsSection.set_spirit_spheres()` (or use `setValue` on the MO_SPIRITBALL `LevelWidget` directly). The `_param_rows["MO_FINGEROFFENSIVE_spheres"]` `_ParamWidget.changed` signal should trigger this. |
