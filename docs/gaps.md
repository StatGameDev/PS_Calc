...
| G52 | [ ] | Dual-wield pipeline: AS_RIGHT (RH ×(50+10×lv)%, battle.c:5923) + AS_LEFT (LH ×(30+10×lv)%, battle.c:5929). Requires separate RH+LH damage branches; only active for `_DUAL_WIELD_JOBS = {12, 4013}`. Deferred from Session P. Needs in-session source read of ~5910–5940 to confirm LH base damage sourcing and exact multiplier application point before implementing. | future |
...
| G54 | [ ] | Double-hit proc branches: TF_DOUBLE (W_DAGGER, 5×lv% chance, battle.c:4867–4874) and GS_CHAINACTION (W_REVOLVER, 5×lv% chance, battle.c:4879–4883). Both set wd.div_=2 → damage_div_fix (battle.c:5567 #ifndef RENEWAL) doubles damage at SkillRatio position. Crits and procs are mutually exclusive: battle.c:4926 `wd.type != BDT_MULTIHIT` gates the crit check. Model as two separate pipeline branches (normal, proc-fires) shown like normal/crit, with effective_crit_chance scaled by (1-proc_chance). Also add DPS stat to SummarySection via AttackDefinition + SelectionStrategy architecture (Markov-upgradeable seam); see docs/aspd.md and Session G54 in session_roadmap.md. | future |
