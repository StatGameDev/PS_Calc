...
| G77 | [ ] | PR_LEXAETERNA ×2 only applies to magic (magic_pipeline.py). Physical BF_WEAPON hits with Lex Aeterna active are not doubled. In Hercules, SC_LEXAETERNA doubles the next hit regardless of type (battle.c). Needs detection in BattlePipeline._run_branch() and ×2 multiplier applied after FinalRateBonus. | SC2 |
| G78 | [ ] | Debuff routing architectural gap: debuffs applied to a player target via TargetStateSection currently run after pvp_status is computed, so cascading stat effects (BLESSING→STR→BATK, CURSE→LUK→BATK, QUAGMIRE→DEX→soft MDEF) are not reflected in pvp_status (= incoming pipeline). Fix: add `collect_target_player_scs()` to TargetStateSection; write to `pvp_eff.player_active_scs` before StatusCalculator runs. Also: new `core/calculators/target_utils.py` with `apply_mob_scs()` to centralise mob-target stat SC application. | Arch |
| G79 | [ ] | Target-side debuffs missing: SC_BLIND (flee×75%), SC_CURSE (luk→0), SC_SLEEP (force-hit), SC_POISON (confirm formula), SC_DONTFORGETME (FLEE+ASPD), SC_MINDBREAKER (MDEF reduction), SC_SIGNUCRUCIS (DEF% vs Undead/Demon), SC_BLESSING-debuff (STR/DEX vs Undead/Demon), SC_QUAGMIRE (AGI+DEX reduction). All need target_state_section UI rows + pipeline implementation. Mob path: apply_mob_scs(). Player path: StatusCalculator via pvp_eff.player_active_scs (after G78). | SC1 |
...
| G80 | [ ] | Player-side debuffs missing: SC_STUN/FREEZE/STONE/SLEEP (incoming force-hit + element for FREEZE/STONE), SC_POISON, SC_DONTFORGETME, SC_ETERNALCHAOS, SC_MINDBREAKER, SC_PROVOKE, SC_SIGNUCRUCIS (Undead/Demon player), SC_BLESSING-debuff (Undead/Demon player), SC_QUAGMIRE. All need player_debuffs_section UI rows + StatusCalculator player_active_scs blocks. No pipeline short-circuit for immobilising SCs (UX decision). | SC2 |
