...
| G72 | [ ] | SC_WHISTLE / SC_HUMMING formulas not source-verified. Both implemented in StatusCalculator (Session M2) but the Hercules grep was explicitly deferred at the time. Formulas may be incorrect — verify against status.c before closing Session R. | R |
| G73 | [ ] | SC_DRUMBATTLE / SC_NIBELUNGEN pipeline timing unconfirmed. Currently applied pre-defense in base_damage.py. If post-defense, should move to mastery_fix.py or active_status_bonus.py. Needs targeted Hercules grep. Related to G51. | R |
...
| G70 | [ ] | Skill List filters everything except Normal Attack until Job is changed once. |
