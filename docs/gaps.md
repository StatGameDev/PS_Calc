...
| G80 | [ ] | Player-side debuffs missing: SC_STUN/FREEZE/STONE/SLEEP (incoming force-hit + element for FREEZE/STONE), SC_POISON, SC_DONTFORGETME (ASPD only — no FLEE), SC_ETERNALCHAOS, SC_MINDBREAKER, SC_PROVOKE, SC_QUAGMIRE. Note: SC_CRUCIS and SC_BLESSING-debuff are mob-only in Hercules (BL_PC blocked) — removed from this list. | SC2 |
