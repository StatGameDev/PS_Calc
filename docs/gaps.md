...
| G84 | [ ] | Font scaling refactor: replace all `font-size: Npx` rules in dark.qss with programmatic font management. Define font tiers (tiny/small/normal/large) as `QFont` objects derived from `app_config.effective_scale()` in app_config.py. Apply via `QApplication.setFont()` at startup and on scale change. Widget constructors that need non-default sizes call a scale-aware helper rather than setting QSS font-size. Prerequisite: removes the need for the debounce workaround in scale adjust — `QApplication.setFont()` is O(1) vs O(n_widgets) for setStyleSheet. Also eliminates the regex QSS transform entirely. | future |
