...
class SizeFix:
    ...
    @staticmethod
    def calculate(weapon: Weapon, build: PlayerBuild, target: Target, current_damage: int, skill: SkillInstance, result: DamageResult) -> None:
        ...
        after_size = current_damage * rate // 100

        multiplier = rate / 100.0 if rate != 100 else 1.0
        ...
        result.add_step(
            ...
            note=note,
            formula=f"current * {rate} // 100   (size_fix table[{target.size}][{weapon.weapon_type}])",
